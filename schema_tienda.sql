...
SELECT * FROM PRODUCTO LEFT JOIN FABRICANTE ON FABRICANTE.CODIGO = PRODUCTO.CODIGO_FABRICANTE WHERE PRODUCTO.PRECIO = FABRICANTE.NOMBRE LIKE 'Lenovo' ORDER BY PRODUCTO.PRECIO DESC LIMIT 1;
...
SELECT PRODUCTO.CODIGO, PRODUCTO.NOMBRE, PRODUCTO.PRECIO FROM PRODUCTO LEFT JOIN FABRICANTE ON FABRICANTE.CODIGO = PRODUCTO.CODIGO_FABRICANTE WHERE PRODUCTO.PRECIO >= FABRICANTE.NOMBRE LIKE 'Lenovo' ORDER BY PRODUCTO.PRECIO DESC LIMIT 1;

-- 41. Llesta tots els productes del fabricador Asus que tenen un preu superior al preu mitjà de tots els seus productes.
SELECT * FROM PRODUCTO LEFT JOIN FABRICANTE ON FABRICANTE.CODIGO = PRODUCTO.CODIGO_FABRICANTE WHERE FABRICANTE.NOMBRE LIKE 'Asus' ORDER BY (PRODUCTO.PRECIO) DESC;
